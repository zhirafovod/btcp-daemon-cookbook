...
def add_peer(torrents, info_hash, peer_id, ip, port):
  ...
  if DEBUG:
    print 'DEBUG: add_peer: torrents: ', torrents, ', info_hash: ', info_hash, ', peer_id: ', peer_id, ', ip: ', ip, ', port: ', port, '\n'
  # If we've heard of this, just add the peer
  if info_hash in torrents:
    # Only add the peer if they're not already in the database
    print 'DEBUG: add_peer: info_hash in torrents: ', peer_id, ip, port
    if (peer_id, ip, port) not in torrents[info_hash]:
      torrents[info_hash].append((peer_id, ip, port))
      print 'DEBUG: add_peer: info_hash in torrents: torrents[info_hash].append((peer_id, ip, port)): ', peer_id, ip, port
  # Otherwise, add the info_hash and the peer
  else:
    torrents[info_hash] = [(peer_id, ip, port)]
    print 'DEBUG: add_peer: info_hash NOT in torrents: torrents[info_hash] = [(peer_id, ip, port)]: ', peer_id, ip, port
...
class RequestHandler(BaseHTTPRequestHandler):
  def do_GET(s):
    ...
    # Decode the request
    print "received request s: %s", s
    print "received request s.path: %s", s.path
    package = decode_request(s.path)
    print "decoded request: ", package
    ...
    print "s.server.torrents adter add_peer: ", s.server.torrents, info_hash, peer_id, ip, port
